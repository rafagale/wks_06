...
public class GestionLibroAutor {
	public static void main(String[] args) {
		...
		do {
			...
			libros = ampliaLibros(libros, posLibro);// amplia los libros si es
													// necesario
			autores = ampliaAutores(autores, posAutor);// amplia los autores si
														// es necesario
	...
	private static Integer creaAutor(Autor[] autores, Integer posAutor) {
		...
		indiceAutor = buscarAutor(autores, nombreAutor);// devuelve la posicion
														// del nombre o -1 si no
														// está
		if (indiceAutor == -1) {
	...
	public static Integer crearLibros(Integer posLibros, Libro[] libros, Autor[] autores) {
		...
		if (indiceAutor == -1) {
			...
			precio = Leer.pedirDouble("Precio del libro?", "^\\d+[.][0-9]?[0-9]?$");// decimal
																					// con
																					// un
																					// máximo
																					// de
																					...
																					// posiciones
																					// decimales
			cantidad = Leer.pedirEntero("Cantidad de libros?", "^\\d+$");// entero
																			// positivo
	...
	public static int buscarTitulo(Libro[] libros, String titulo) {
		// devuelve el indice de la posición donde está ese título o el valor -1
		// si no lo encuentra
	...
	public static void modificarAutor(Autor[] autores, String nombre) {
		...
		if (indiceAutor != -1) {
			...
			if (!nombre.equals("*")) {
				indice2 = buscarAutor(autores, nombre);// verificamos que el
														// nuevo nombre no está
														// repetido
				if (indice2 != -1) {// nombre ya existente
					Leer.mostrarEnPantalla(
							"El nuevo nombre ya está en la lista, posición " + indice2 + " No se puede cambiar");
				} else {// nombre nuevo
					autor.setNombre(nombre);
				}
			}
			email = Leer.pedirCadena("Nuevo email (* no cambiar)",
					"^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$|[*]");
			if (!email.equals("*")) {
	...
	public static void modificarLibro(Libro[] libros, String titulo, Autor[] autores) {
		...
		if (indiceLibro != -1) {
			...
			if (!titulo.equals("*")) {// si no se teclea * se mantiene el título
				indice2 = buscarTitulo(libros, titulo);// verificamos que el
														// nuevo título no está
														// repetido
				if (indice2 != -1) {// el título ya existe (puede ser el mismo)
					Leer.mostrarEnPantalla(
							"El nuevo título ya está en la lista, posición " + indice2 + " No se puede cambiar");
				} else {// el nuevo título es nuevo de verdad
					libro.setTitulo(titulo);
				}
			}
			precio = Leer.pedirDouble("Precio " + libro.getPrecio() + " Nuevo precio del libro ",
					"^\\d+[.][0-9]?[0-9]?$");
