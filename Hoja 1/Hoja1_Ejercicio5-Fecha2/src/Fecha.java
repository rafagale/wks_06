...
public class Fecha {
	...
	public int diferenciaFecha(Fecha fecha2) {
		...
		diasTotalesActual= diasDesdeElInicio();
		diasTotalesFecha2 = fecha2.diasDesdeElInicio();
	...
	private Integer diasDesdeElInicio() {
		int diferencia = 0;
		int diasTotales=0;

		int[] meses = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

		for (int i = 0; i <= anio - 1; i++) {
			//if (esBisiesto(i)) {//*************************
			if (esBisiesto()) {// se invoca desde el objeto que ha llamado a diasDesdeElInicio(Fecha date)
