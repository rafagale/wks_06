...
public class Colegio {
	...
	public static void main(String[] args) {
		...
		for (int j = 0; j < alumnos.length; j++) {
			alumnos[j].seMatriculaEn(asignaturas[j]);
			System.out.println();
	...
	public static void crearAlumnos(Alumno[] alumnos) {
		...
		for (int i = 0; i < 1; i++) {
			fechaNacimiento = new Fecha(rand.nextInt(28) + 1, rand.nextInt(11) + 1, rand.nextInt(2000 - 1995 + 1) + 1995);
	...
	public static void crearAsignaturas(Asignatura[] asignaturas) {
		String[] nombres = { "Matematicas", "Lengua", "Ingles", "Fisica", "Quimica", "Dibujo tecnico", "Recreo", "Anatomia", "Fisiologia", "Informatica" };
		Random rand = new Random();
		for (int i = 0; i < 2; i++) {
